...
void handle_gpu_bcast_cpu_gpu_event(
		...
		tw_lp * lp)
{
	//	printf("Received control message from CPU on rank %d to send to rank %d.\n", ns->my_pe_num, m->msgId.dest_id);
	proc_msg gpu_gpu_msg;
	gpu_gpu_msg.proc_event_type = COLL_BCAST_GPU_GPU;
	gpu_gpu_msg.src = lp->gid;
	gpu_gpu_msg.msgId = m->msgId;
	gpu_gpu_msg.iteration = m->iteration;
	model_net_event(net_id, "gpu_direct", pe_to_lpid_gpu(m->msgId.dest_id, m->msgId.job), m->msgId.size, rdma_delay + nic_delay, sizeof(proc_msg), &gpu_gpu_msg, 0, NULL, lp);
}
...
void handle_gpu_bcast_gpu_gpu_event(
		...
		tw_lp *lp)
{
//	printf("Received broadcast message on GPU on rank %d.\n", ns->my_pe_num); 
	proc_msg gpu_cpu_msg;
	gpu_cpu_msg.proc_event_type = COLL_BCAST;
	gpu_cpu_msg.src = lp->gid;
	gpu_cpu_msg.msgId = m->msgId;
	gpu_cpu_msg.iteration = m->iteration;
	model_net_event(net_id, "control", pe_to_lpid(ns->my_pe_num, m->msgId.job), 16, 0, sizeof(proc_msg), &gpu_cpu_msg, 0, NULL, lp);
}
...
//	printf("Received control message from CPU on rank %d to send to rank %d.\n", ns->my_pe_num, m->msgId.dest_id);
	proc_msg gpu_gpu_msg;
	gpu_gpu_msg.proc_event_type = COLL_REDUCTION_GPU_GPU;
	gpu_gpu_msg.src = lp->gid;
	gpu_gpu_msg.msgId = m->msgId;
	gpu_gpu_msg.iteration = m->iteration;
	model_net_event(net_id, "gpu_direct", pe_to_lpid_gpu(m->msgId.dest_id, m->msgId.job), m->msgId.size, rdma_delay + nic_delay * ((m->msgId.size>16)?1:0) + MPI_INTERNAL_DELAY, sizeof(proc_msg), &gpu_gpu_msg, 0, NULL, lp); 
}
...
void handle_gpu_reduction_gpu_gpu_event(
		...
		tw_lp *lp)
{
//	printf("Received reduction message on GPU on rank %d.\n", ns->my_pe_num);
	proc_msg gpu_cpu_msg;
	gpu_cpu_msg.proc_event_type = COLL_REDUCTION;
	gpu_cpu_msg.src = lp->gid;
	gpu_cpu_msg.msgId = m->msgId;
	gpu_cpu_msg.iteration = m->iteration;
	model_net_event(net_id, "control", pe_to_lpid(ns->my_pe_num, m->msgId.job), 16, 0, sizeof(proc_msg), &gpu_cpu_msg, 0, NULL, lp);
...
//	printf("Received control message from CPU on rank %d to send to rank %d.\n", ns->my_pe_num, m->msgId.dest_id);
	proc_msg gpu_gpu_msg;
	gpu_gpu_msg.proc_event_type = COLL_A2A_GPU_GPU;
	gpu_gpu_msg.src = lp->gid;
	gpu_gpu_msg.msgId = m->msgId;
	gpu_gpu_msg.iteration = m->iteration;

	proc_msg gpu_local_event;
	gpu_local_event.proc_event_type = COLL_A2A_SEND_DONE_GPU;
	gpu_local_event.msgId = m->msgId;
	gpu_local_event.iteration = m->iteration;	

	model_net_event(net_id, "gpu_direct", pe_to_lpid_gpu(m->msgId.dest_id, m->msgId.job), m->msgId.size, rdma_delay + nic_delay, sizeof(proc_msg), &gpu_gpu_msg, sizeof(proc_msg), &gpu_local_event, lp);
}
...
void handle_gpu_a2a_gpu_gpu_event(
		...
		tw_lp *lp)
{
//	printf("Received all2all message on GPU on rank %d.\n", ns->my_pe_num);
	proc_msg gpu_cpu_msg;
	gpu_cpu_msg.proc_event_type = COLL_A2A;
	gpu_cpu_msg.src = lp->gid;
	gpu_cpu_msg.msgId = m->msgId;
	gpu_cpu_msg.iteration = m->iteration;
	model_net_event(net_id, "control", pe_to_lpid(ns->my_pe_num, m->msgId.job), 16, 0, sizeof(proc_msg), &gpu_cpu_msg, 0, NULL, lp);
...
void handle_gpu_a2a_send_done_event(
		...
		tw_lp *lp)
{
//	printf("Informing CPU of send done on rank %d.\n", ns->my_pe_num);
	proc_msg gpu_cpu_msg;
	gpu_cpu_msg.proc_event_type = COLL_A2A_SEND_DONE;
	gpu_cpu_msg.executed.taskid = m->msgId.req_id;
	model_net_event(net_id, "control", pe_to_lpid(ns->my_pe_num, m->msgId.job), 16, 0, sizeof(proc_msg), &gpu_cpu_msg, 0, NULL, lp);
}
...
//	printf("Received control message from CPU on rank %d to send to rank %d.\n", ns->my_pe_num, m->msgId.dest_id);
	proc_msg gpu_gpu_msg;
	gpu_gpu_msg.proc_event_type = COLL_ALLGATHER_GPU_GPU;
	gpu_gpu_msg.src = lp->gid;
	gpu_gpu_msg.msgId = m->msgId;
	gpu_gpu_msg.iteration = m->iteration;

	proc_msg gpu_local_event;
	gpu_local_event.proc_event_type = COLL_ALLGATHER_SEND_DONE_GPU;
	gpu_local_event.msgId = m->msgId;
	gpu_local_event.iteration = m->iteration;	
...
void handle_gpu_allgather_gpu_gpu_event(
		...
		tw_lp *lp)
{
//	printf("Received allgather message on GPU on rank %d.\n", ns->my_pe_num);
	proc_msg gpu_cpu_msg;
	gpu_cpu_msg.proc_event_type = COLL_ALLGATHER;
	gpu_cpu_msg.src = lp->gid;
	gpu_cpu_msg.msgId = m->msgId;
	gpu_cpu_msg.iteration = m->iteration;
	model_net_event(net_id, "control", pe_to_lpid(ns->my_pe_num, m->msgId.job), 16, 0, sizeof(proc_msg), &gpu_cpu_msg, 0, NULL, lp);
...
//	printf("Informing CPU of send done on rank %d.\n", ns->my_pe_num);
	proc_msg gpu_cpu_msg;
	gpu_cpu_msg.proc_event_type = COLL_ALLGATHER_SEND_DONE;
	gpu_cpu_msg.executed.taskid = m->msgId.req_id;
	model_net_event(net_id, "control", pe_to_lpid(ns->my_pe_num, m->msgId.job), 16, 0, sizeof(proc_msg), &gpu_cpu_msg, 0, NULL, lp);
}
...
void enqueue_coll_msg(
	...
	bool isEager = ((size <= eager_limit) || useEager) && msgId->isGPUDirect == 0;
	...
	else if(msgId->isGPUDirect && lookUpTable[index].remote_event == COLL_A2A)
	{
		proc_msg control_cpu_gpu;
		control_cpu_gpu.proc_event_type = COLL_A2A_CPU_GPU;
		control_cpu_gpu.src = lp->gid;
		control_cpu_gpu.msgId.size = size;
		control_cpu_gpu.msgId.pe = msgId->pe;
		control_cpu_gpu.msgId.id = msgId->id;
		control_cpu_gpu.msgId.dest_id = dest;
		control_cpu_gpu.msgId.job = ns->my_job;
		control_cpu_gpu.msgId.comm = msgId->comm;
		control_cpu_gpu.msgId.seq = seq;
		control_cpu_gpu.msgId.req_id = ns->my_pe->currentCollTask;
		control_cpu_gpu.iteration = iter;
		model_net_event(net_id, "control", pe_to_lpid_gpu(ns->my_pe_num, ns->my_job), 16, gpudirect_delay, sizeof(proc_msg), &control_cpu_gpu, 0, NULL, lp);
		if(!isEager) {
			b->c17 = 1;
			it->second.pop_front();
			if(it->second.size() == 0) {
	...
	else if(msgId->isGPUDirect && lookUpTable[index].remote_event == COLL_ALLGATHER)
	{
		proc_msg control_cpu_gpu;
		control_cpu_gpu.proc_event_type = COLL_ALLGATHER_CPU_GPU;
		control_cpu_gpu.src = lp->gid;
		control_cpu_gpu.msgId.size = size;
		control_cpu_gpu.msgId.pe = msgId->pe;
		control_cpu_gpu.msgId.id = msgId->id;
		...
		tw_lp * lp)
{
	...
	if(it == ns->my_pe->pendingRCollMsgs.end() || it->second.front() == -1) {
		...
		if(t->myEntry.msgId.coll_type == OTF2_COLLECTIVE_OP_ALLTOALL && t->myEntry.msgId.isGPUDirect == 1)
		{
//			printf("Performaing GPUDirect version in coll_recv_post_event.\n");
			proc_msg control_cpu_gpu;
			control_cpu_gpu.proc_event_type = COLL_A2A_CPU_GPU;
			control_cpu_gpu.src = lp->gid;
			control_cpu_gpu.msgId.size = t->myEntry.msgId.size;
			control_cpu_gpu.msgId.pe = t->myEntry.msgId.pe;
			control_cpu_gpu.msgId.id = t->myEntry.msgId.id;
			control_cpu_gpu.msgId.dest_id = m->msgId.pe;
			control_cpu_gpu.msgId.job = ns->my_job;
			control_cpu_gpu.msgId.comm = ns->my_pe->currentCollComm;
			control_cpu_gpu.msgId.seq = ns->my_pe->currentCollSeq;
			control_cpu_gpu.msgId.req_id = ns->my_pe->currentCollTask;
			control_cpu_gpu.iteration = ns->my_pe->currIter;
			model_net_event(net_id, "control", pe_to_lpid_gpu(ns->my_pe_num, ns->my_job), 16, gpudirect_delay, sizeof(proc_msg), &control_cpu_gpu, 0, NULL, lp);
		}
		else if(t->myEntry.msgId.coll_type == OTF2_COLLECTIVE_OP_ALLGATHER && t->myEntry.msgId.isGPUDirect == 1)
		{
//			printf("Performing GPUDirect version of AllGather.\n");
			proc_msg control_cpu_gpu;
			control_cpu_gpu.proc_event_type = COLL_ALLGATHER_CPU_GPU;
			control_cpu_gpu.src = lp->gid;
			control_cpu_gpu.msgId.size = t->myEntry.msgId.size;
...
void perform_collective(
	...
	if(t->myEntry.msgId.coll_type == OTF2_COLLECTIVE_OP_BCAST) {
		...
		perform_allreduce(ns, taskid, lp, m, b, 0);
			...
			(t->myEntry.msgId.size > TRACER_A2A_ALG_CUTOFF || t->myEntry.msgId.isGPUDirect)) {
		...
		perform_a2a_blocked(ns, taskid, lp, m, b, 0);
			...
			(t->myEntry.msgId.size * g.members.size() > TRACER_ALLGATHER_ALG_CUTOFF || t->myEntry.msgId.isGPUDirect)) {
...
void perform_bcast(
	...
	tw_stime gpu_copy_time = gpu_copy_delay + gpu_copy_per_byte * t->myEntry.msgId.size;
	if(gpu_copy_enabled)
		delay +=  gpu_copy_time;
	m->model_net_calls = 0;
	for(int i = 0; i < numValidChildren; i++) {
		int dest = g.members[myChildren[i]];
		if(t->myEntry.msgId.isGPUDirect == 0)
		{
		...
		{
//			printf("Sending control message to GPU from rank %d\n", ns->my_pe_num);
			proc_msg m_control;
			m_control.proc_event_type = COLL_BCAST_CPU_GPU;
			m_control.src = lp->gid;
			m_control.msgId.size = t->myEntry.msgId.size;
			m_control.msgId.pe = t->myEntry.msgId.pe;
			m_control.msgId.id = t->myEntry.msgId.id;
			m_control.msgId.job = ns->my_job;
			m_control.msgId.comm = t->myEntry.msgId.comm;
			m_control.msgId.seq = ns->my_pe->currentCollSeq;
			m_control.iteration = ns->my_pe->currIter;
			m_control.msgId.dest_id = dest;
			model_net_event(net_id, "control", pe_to_lpid_gpu(ns->my_pe_num, ns->my_job), 16, gpudirect_delay, sizeof(struct proc_msg), (const void *)&m_control, 0, NULL, lp);
		}
	}
...
//			printf("Sending control message to GPU from rank %d\n", ns->my_pe_num);
			proc_msg m_control;
			m_control.proc_event_type = COLL_REDUCTION_CPU_GPU;
			m_control.src = lp->gid;
			m_control.msgId.size = t->myEntry.msgId.size;
			m_control.msgId.pe = t->myEntry.msgId.pe;
			m_control.msgId.id = t->myEntry.msgId.id;
...
void perform_a2a(
	...
	if(ns->my_pe->currentCollPartner < ns->my_pe->currentCollSize - 1) {
		...
		if(t->myEntry.msgId.size > eager_limit || t->myEntry.msgId.isGPUDirect) {
			m->model_net_calls++;
